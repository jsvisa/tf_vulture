...
class VultureLibCurlProxy : public VultureLibCurl {
 public:
  static VultureLibCurlProxy* Load() {
    static VultureLibCurlProxy* libcurl = []() -> VultureLibCurlProxy* {
      curl_global_init(CURL_GLOBAL_ALL);
      return new VultureLibCurlProxy;
    }();
    return libcurl;
  }
...
VultureHttpRequest::VultureHttpRequest() : VultureHttpRequest(VultureLibCurlProxy::Load()) {}

VultureHttpRequest::VultureHttpRequest(VultureLibCurl* libcurl, Env* env)
  ...
  TF_CURL_LOG_WITH_CONTEXT_IF_ERROR(
      libcurl_->curl_easy_setopt(curl_, CURLOPT_XFERINFOFUNCTION,
                                 &VultureHttpRequest::ProgressCallback),
...
VultureHttpRequest::~VultureHttpRequest() {
  if (curl_headers_) {
...
string VultureHttpRequest::EscapeString(const string& str) {
...
void VultureHttpRequest::SetUri(const string& uri) {
  CheckNotSent();
  is_uri_set_ = true;
  uri_ = uri;
  TF_CURL_LOG_WITH_CONTEXT_IF_ERROR(
...
void VultureHttpRequest::SetRange(uint64 start, uint64 end) {
  CheckNotSent();
  TF_CURL_LOG_WITH_CONTEXT_IF_ERROR(
...
void VultureHttpRequest::AddHeader(const string& name, const string& value) {
  CheckNotSent();
  curl_headers_ = libcurl_->curl_slist_append(
      curl_headers_, strings::StrCat(name, ": ", value).c_str());
}

void VultureHttpRequest::AddResolveOverride(const string& hostname, int64 port,
...
void VultureHttpRequest::AddAuthBearerHeader(const string& auth_token) {
  CheckNotSent();
  if (!auth_token.empty()) {
...
void VultureHttpRequest::SetRequestStats(RequestStats* stats) {
  CheckNotSent();
  CHECK(stats_ == nullptr) << "SetRequestStats already called";
  stats_ = stats;
}

void VultureHttpRequest::SetDeleteRequest() {
  CheckNotSent();
  CheckMethodNotSet();
  is_method_set_ = true;
...
Status VultureHttpRequest::SetPutFromFile(const string& body_filepath,
                                       size_t offset) {
  CheckNotSent();
  CheckMethodNotSet();
  is_method_set_ = true;
...
void VultureHttpRequest::SetPutEmptyBody() {
  CheckNotSent();
  CheckMethodNotSet();
  is_method_set_ = true;
  method_ = RequestMethod::kPut;
  ...
  TF_CURL_LOG_WITH_CONTEXT_IF_ERROR(
      libcurl_->curl_easy_setopt(curl_, CURLOPT_READFUNCTION,
                                 &VultureHttpRequest::ReadCallback),
      "Setting read callback");
}

void VultureHttpRequest::SetPostFromBuffer(const char* buffer, size_t size) {
  CheckNotSent();
  CheckMethodNotSet();
  is_method_set_ = true;
...
void VultureHttpRequest::SetPostEmptyBody() {
  CheckNotSent();
  CheckMethodNotSet();
  is_method_set_ = true;
  method_ = RequestMethod::kPost;
...
void VultureHttpRequest::SetResultBuffer(std::vector<char>* out_buffer) {
  CheckNotSent();
  ...
  TF_CURL_LOG_WITH_CONTEXT_IF_ERROR(
      libcurl_->curl_easy_setopt(curl_, CURLOPT_WRITEFUNCTION,
                                 &VultureHttpRequest::WriteCallback),
      "Setting write callback");
}

void VultureHttpRequest::SetResultBufferDirect(char* buffer, size_t size) {
  ...
  TF_CURL_LOG_WITH_CONTEXT_IF_ERROR(
      libcurl_->curl_easy_setopt(curl_, CURLOPT_WRITEFUNCTION,
                                 &VultureHttpRequest::WriteCallbackDirect),
      "Setting write callback");
}

bool VultureHttpRequest::IsDirectResponse() const {
  return direct_response_.buffer_ != nullptr;
}

size_t VultureHttpRequest::WriteCallbackDirect(const void* ptr, size_t size,
                                            size_t nmemb, void* userdata) {
  CHECK(ptr != nullptr);
  auto that = reinterpret_cast<VultureHttpRequest*>(userdata);
...
size_t VultureHttpRequest::GetResultBufferDirectBytesTransferred() {
  CHECK(direct_response_.buffer_ != nullptr);
  return direct_response_.bytes_transferred_;
}

void VultureHttpRequest::SetTimeouts(uint32 connection, uint32 inactivity,
                                  uint32 total) {
  CheckNotSent();
...
size_t VultureHttpRequest::WriteCallback(const void* ptr, size_t size,
                                      size_t nmemb, void* this_object) {
  CHECK(ptr);
  auto that = reinterpret_cast<VultureHttpRequest*>(this_object);
...
size_t VultureHttpRequest::ReadCallback(void* ptr, size_t size, size_t nmemb,
                                     FILE* this_object) {
  CHECK(ptr);
  auto that = reinterpret_cast<VultureHttpRequest*>(this_object);
...
size_t VultureHttpRequest::HeaderCallback(const void* ptr, size_t size,
                                       size_t nmemb, void* this_object) {
  CHECK(ptr);
  auto that = reinterpret_cast<VultureHttpRequest*>(this_object);
...
Status VultureHttpRequest::Send() {
  CheckNotSent();
  ...
  TF_CURL_RETURN_WITH_CONTEXT_IF_ERROR(
      libcurl_->curl_easy_setopt(curl_, CURLOPT_HEADERFUNCTION,
                                 &VultureHttpRequest::HeaderCallback),
...
void VultureHttpRequest::CheckMethodNotSet() const {
  CHECK(!is_method_set_) << "HTTP method has been already set.";
}

void VultureHttpRequest::CheckNotSent() const {
  CHECK(!is_sent_) << "The request has already been sent.";
}

StringPiece VultureHttpRequest::GetResponse() const {
...
string VultureHttpRequest::GetResponseHeader(const string& name) const {
  const auto& header = response_headers_.find(name);
  return header != response_headers_.end() ? header->second : "";
}

uint64 VultureHttpRequest::GetResponseCode() const { return response_code_; }

// Cancels the transmission if no progress has been made for too long.
int VultureHttpRequest::ProgressCallback(void* this_object, curl_off_t dltotal,
                                      curl_off_t dlnow, curl_off_t ultotal,
                                      curl_off_t ulnow) {
  auto that = reinterpret_cast<VultureHttpRequest*>(this_object);
