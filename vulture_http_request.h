...
#ifndef TENSORFLOW_CORE_PLATFORM_VULTURE_VULTURE_HTTP_REQUEST_H_
#define TENSORFLOW_CORE_PLATFORM_VULTURE_VULTURE_HTTP_REQUEST_H_
...
class VultureLibCurl;  // libcurl interface as a class, for dependency injection.
...
///
/// For example:
///   std::shared_ptr<HttpRequest> request(http_request_factory->Create());
///   request->SetUri("http://www.google.com");
///   request->SetResultsBuffer(out_buffer);
///   request->Send();
class VultureHttpRequest : public HttpRequest {
 public:
  class Factory : public HttpRequest::Factory {
   public:
    virtual ~Factory() {}
    virtual HttpRequest* Create() { return new VultureHttpRequest(); }
  };

  VultureHttpRequest();
  explicit VultureHttpRequest(VultureLibCurl* libcurl)
      : VultureHttpRequest(libcurl, Env::Default()) {}
  VultureHttpRequest(VultureLibCurl* libcurl, Env* env);
  ~VultureHttpRequest() override;
 ...
 private:
  ...
  TF_DISALLOW_COPY_AND_ASSIGN(VultureHttpRequest);
};
...
class VultureLibCurl {
 public:
  virtual ~VultureLibCurl() {}
